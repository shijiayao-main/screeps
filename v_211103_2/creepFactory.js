...
var creepFactory = {
    ...
    */
    run: function (roleWorker, workerCount, roleUpgrader, upgraderCount, roleHarvester, harvesterCount) {
        ...
        var needBuildCreep = needToBuildCreep(harvesterCount, upgraderCount, workerCount);
        
        if(!needBuildCreep) {
            Game.spawns['Spawn1'].memory.neededEnergyToBuild = false;
            Log.i('don not need to build creep');
            return;
        }
        if (couldUseEnergyNum < neededEnergy) {
            Log.i('fuck i need energy!! need: ' + neededEnergy + ', coule use: ' + couldUseEnergyNum);
            return;
        }
...
function needToBuildCreep(harvesterCount, upgraderCount, workerCount) {
    if (harvesterCount <= 0) {
...
function buildBody(workerCount, harvesterCount) {
    ...
    if (allEnergyNum <= 300 || (workerCount + harvesterCount) <= 3) {
        body = [
            WORK,
            CARRY,
            MOVE
